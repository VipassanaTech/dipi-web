...
$q = "select bm_id, bm_query, bm_letter from dh_bulk_mail where DATE_FORMAT(bm_schedule_date, '%Y-%m-%d') = CURDATE() and bm_processed=0 and bm_deleted=0";
$result = db_query($q);
while ($row = $result->fetchAssoc()) 
{
   echo "Processing Bulk Mail ID ".$row['bm_id']."\n";
   $q = "update dh_bulk_mail set bm_processed=2 where bm_id=".$row['bm_id'];
   db_query($q);
   $result2 = db_query($row['bm_query']);
   while ($row2 = $result2->fetchAssoc()) 
   {
      echo "Pushing ".$row2['a_id']."\n";
      $data = array('bulk_mail_id' => $row['bm_id'], 'applicant_id' => $row2['a_id'], 'letter_id' => $row['bm_letter']);
